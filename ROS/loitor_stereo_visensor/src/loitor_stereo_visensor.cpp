...
void* imu_data_stream(void *)
{
	...
	while(!visensor_Close_IMU_viewer)
	{
		if(visensor_imu_have_fresh_data())
			...
			imu_time.nsec=visensor_imudata_pack.system_time.tv_usec;
			...
			imu_msg.angular_velocity.x=visensor_imudata_pack.rx;
			imu_msg.angular_velocity.y=visensor_imudata_pack.ry;
			imu_msg.angular_velocity.z=visensor_imudata_pack.rz;
...
{ 
	...
	// argv[1]="/home/di-tech/workspace/loitor_cam2-imulib/build/Loitor_VISensor_Setups.txt"
	...
	while (ros::ok()) 
	{
		...
		if(visensor_cam_selection==0)
		{
			...
			msg_time.nsec=left_stamp.tv_usec;
			t_left.header.stamp = msg_time;
			ros::Time msg1_time;
			msg1_time.sec=right_stamp.tv_sec;
			msg1_time.nsec=right_stamp.tv_usec;
